...
_MAX_NUM_SAMPLES_TO_CACHE = 150000


def make_get_tensors_fn(output_tensors):
  """Creates a function that outputs a collection of tensors from the dataset."""

  def _get_fn(data):
    """Gets tensors by name."""
...
def make_get_and_cast_tensors_fn(output_tensors):
  """Creates a function that gets and casts a set of tensors from the dataset.
...
def compose_preprocess_fn(*functions):
  """Composes two or more preprocessing functions.
...
class ImageData(object):
  ...
  def __init__(self,
    ...
    Args:
      ...
      filter_fn: optional, function to filter the examples to use in the
        dataset.
      image_decoder: optional, function to decode image.
      num_channels: optional, number of channels in the dataset image.
    """
  ...
  def get_tf_data(self,
    ...
    Args:
      ...
      num_train_examples: optional number of examples to take for training.
    ...
    # Obtain tf.data object.
    # We shuffle when not for eval, it's important to not shuffle before
    # a subset of data is retrieved.
    data = self._get_dataset_split(
        split_name=split_name,
        shuffle_files=not (for_eval or num_train_examples))

    if not for_eval:
      # Dataset filtering priority: (1) filter_fn; (2) num_train_examples.
      if self._filter_fn and num_train_examples:
        raise ValueError(
            'You must not set both filter_fn and num_train_examples.')
      if self._filter_fn:
        tf.logging.warning(
            'You are filtering the dataset. Notice that this may hurt your '
            'throughput, since examples still need to be decoded, and may '
            'make the result of get_num_samples() inaccurate. '
            'num_train_examples is ignored for filtering, but only used for '
      ...
      elif num_train_examples:
        # Deterministic for same dataset version.
        data = data.take(num_train_examples)
      ...
      if num_samples <= _MAX_NUM_SAMPLES_TO_CACHE:
        data = data.cache()

    # Repeat data `epochs` time or indefinitely if `epochs` is None.
  ...
  def _get_dataset_split(self, split_name, shuffle_files=False):
    """Returns the Dataset object for the given split name.
