...
class Caltech101(base.ImageTfdsData):
  ...
  def __init__(self, data_dir=None):
    ...
    """Initializer for the Caltech101 class.

    Args:
      data_dir: optional, data_dir used to store the dataset
        (will download the dataset into this folder if running locally)
    """
    dataset_builder = tfds.builder("caltech101:3.*.*", data_dir=data_dir)
    dataset_builder.download_and_prepare()

    # Create a dict with example counts for each split.
    ...
    # Define dataset specific train/val/trainval/test splits.
    tfds_splits = {
